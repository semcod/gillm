...
from gllm.capture import capture_primary_rgb_wayland_fallback
from gllm.focus import resolve_active_os_strategy
from gllm.focus.strategy import FocusOutcome, KeySequence
from gllm.injection import Injector
from gllm.intents import gui_contract, validate_contract_runtime
from gllm.nlp_bridge import NLPBridgeClient

logger = logging.getLogger("gllm.orchestrator")
...
class DriveOrchestrator:
    ...
    def focus_target_window(self, window_name_hints: tuple[str, ...]) -> FocusOutcome:
        ...
        # @intract.v1 scope:method intent:gui_focus priority:3 domain:gui input:window_name_hints output:FocusOutcome effect:focus_window meaning:"Focus the target window using active OS strategy."
        """
    ...
    def inject_text(
        ...
        # @intract.v1 scope:method intent:gui_inject priority:3 domain:gui input:text,ide,submit,dry_run output:result effect:inject_keys meaning:"Injects keystrokes or types text into the IDE."
        """
    ...
    def capture_screenshot(self, scale: float | None = None) -> Any:
        ...
        # @intract.v1 scope:method intent:gui_capture priority:3 domain:gui input:scale output:CapturedImage effect:capture_primary_rgb_wayland_fallback meaning:"Captures screenshot of primary display."
        """
    ...
    def execute_step(self, step: dict[str, Any], dry_run: bool = False) -> dict[str, Any]:
        ...
        # @intract.v1 scope:method intent:gui_execute_step priority:3 domain:gui input:step,dry_run output:result effect:execute_step meaning:"Executes one GUI control DSL step."
        """
    ...
    def execute_workflow(self, steps: list[dict[str, Any]], dry_run: bool = False) -> list[dict[str, Any]]:
        ...
        # @intract.v1 scope:method intent:gui_execute_workflow priority:3 domain:gui input:steps,dry_run output:results effect:execute_workflow meaning:"Executes a list of GUI actions sequentially."
        """
        results = []
        for step in steps:
    ...
    def drive_natural_language(self, command: str, dry_run: bool = False) -> list[dict[str, Any]]:
        ...
        # @intract.v1 scope:method intent:gui_drive_nlp priority:3 domain:gui input:command,dry_run output:results effect:drive_natural_language meaning:"Parses and runs natural language GUI command."
        """
