"""CLI for dsl2gillm — dual mode: legacy (-c) + subcommands."""
...
from dsl2gillm.codec import envelope_from_bytes, envelope_to_bytes, parse_text, roundtrip_text
from dsl2gillm.events import EventStore
from dsl2gillm.schema_registry import validate_schemas

_SUBCOMMANDS = {"validate-schema", "encode", "decode", "replay", "run", "roundtrip", "codegen"}
...
def _main_subcommand(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dsl2gillm")
    sub = parser.add_subparsers(dest="cmd", required=True)

    sub.add_parser("validate-schema")
    sub.add_parser("codegen", help="Regenerate pydantic models.py from schemas")
...
def _main_legacy(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="dsl2gillm",
        description="Gillm control DSL (HEALTH, PARSE, EXECUTE, …)",
    )
    ...
    parser.add_argument("--file", default="", help="Default workflow file")
...
def _handle_subcommand(args: argparse.Namespace) -> int:
    ...
    if args.cmd == "codegen":
        from dsl2gillm.codegen import generate_models
    ...
    if args.cmd == "encode":
        ...
        if args.format == "json":
            from dsl2gillm.codec import envelope_to_json
    ...
    if args.cmd == "decode":
        ...
        if args.format == "json":
            from dsl2gillm.codec import envelope_from_json
    ...
    if args.cmd == "replay":
        store = EventStore.for_workdir(Path(args.file))
