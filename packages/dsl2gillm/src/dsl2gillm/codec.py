...
from dsl2gillm.pb_codec import decode_protobuf, encode_protobuf
from dsl2gillm.schema_registry import schema_for_verb


def _validate_with_pydantic(payload: dict[str, Any], verb: str) -> None:
    try:
        from dsl2gillm.models import MODEL_BY_VERB
    except ImportError:
        return
    model_cls = MODEL_BY_VERB.get(verb)
...
def roundtrip_text(line: str, *, default_file: str | None = None) -> str:
    payload = parse_text(line, default_file=default_file)
