...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="gillm:// URI → dsl2gillm")
    ...
    resolve = sub.add_parser("resolve", help="NL → gillm:// URIs")
    ...
    if args.cmd == "resolve":
        hits = nlp2uri(args.prompt, file=default_file)
        if args.json:
